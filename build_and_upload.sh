...
sed -e "s/version = \"[[:digit:]]\.[[:digit:]]\.[[:digit:]]\"/version = \"${VERSION}\"/g" src/utils/parsers/args_parser.py | \
  tee src/utils/parsers/args_parser.temp.py && \
...
if python3 setup.py sdist bdist_wheel 2>&1 ; then
